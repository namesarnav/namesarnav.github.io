...
interface WarpTextTuneableProps {
  text: string;
  color: string;
  fontSize: string | number;
  fontWeight: string | number;
...
const buildTextCanvas = ({
  ...
  const originX = align === "left" ? width * 0.03 : width / 2;
  lines.forEach((line, index) => drawLine(ctx, line, originX, startY + index * lineHeight, letterSpacing, align));
  return canvas;
};

const syncUniforms = (program: Program, props: WarpTextTuneableProps) => {
...
export default function WarpText({
  ...
  // Keep the latest props available synchronously for the WebGL effect's closures below.
  ...
  useEffect(() => {
    ...
    let reduceMotion = window.matchMedia ? window.matchMedia("(prefers-reduced-motion: reduce)").matches : false;
    let rasterVersion = 0;
    let cleanupFns: (() => void)[] = [];
    ...
    const rasterize = async () => {
      ...
      texture.image = buildTextCanvas({ container, width: rect.width, height: rect.height, dpr, props: propsRef.current });
      texture.needsUpdate = true;
      renderOnce();
    };

    const resize = () => {
      if (disposed || contextLost || !renderer || !gl || !program) return;
      const rect = container.getBoundingClientRect();
      if (rect.width <= 0 || rect.height <= 0) return;
    ...
    const onReducedMotion = (event: MediaQueryListEvent) => {
      ...
      if (program) program.uniforms.uMotion.value = reduceMotion ? 0 : 1;
      renderOnce();
    };
    ...
    // Pointer events come from the whole hero area, so the lens follows the cursor
    // even though the canvas itself sits behind other hero content.
    window.addEventListener("pointermove", onPointerMove);
    canvas.addEventListener("pointerleave", onPointerLeave);
    canvas.addEventListener("webglcontextlost", onContextLost, false);
    document.addEventListener("visibilitychange", onVisibility);
    if (mediaQuery && mediaQuery.addEventListener) mediaQuery.addEventListener("change", onReducedMotion);
    ...
    cleanupFns.push(() => {
      ...
      if (mediaQuery && mediaQuery.removeEventListener) mediaQuery.removeEventListener("change", onReducedMotion);
      if (!contextLost && gl) {
        try {
  ...
  return (
    <div
      ...
      style={Object.assign({ position: "relative", display: "block", width: "100%", height: "100%", overflow: "hidden", isolation: "isolate" }, style)}
      role="img"
      aria-label={text}
    >
      <h1 className="avp-sr-only">{text}</h1>
    </div>
  );
}
